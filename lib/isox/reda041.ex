defmodule Isox.Reda041 do
  @moduledoc """
  Aviso de mudança no cadastro de um participante.

  Versão 1.7. Diz o que mudou, com valor antigo e valor novo de cada
  campo.

  `Rcrd.Othr` é `[1..3]` no schema real (`maxOccurs="3"`, sem `minOccurs`,
  logo 1 implícito), não ilimitado: no máximo uma alteração por campo, e
  só existem três campos alteráveis (`"MODP"`, `"NOME"` e `"NOMR"`).

  Ao contrário de `StsRsnInf` e `TxInfAndSts` nos outros módulos, aqui a
  lista é o próprio conteúdo da mensagem, então é modelada como lista de
  verdade, não simplificada para um item. `encode/3` não precisa validar o
  limite: o round-trip do `confirm/2` já rejeita 0 ou mais de 3.
