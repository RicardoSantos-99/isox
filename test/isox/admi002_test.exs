defmodule Isox.Admi002Test do
  use ExUnit.Case, async: true

  alias Isox.{Admi002, AppHdr}
  ...
  test "monta e volta pra struct, só com o obrigatório" do
    assert {:ok, xml} = Admi002.encode(@message, @header, :v1_5)
    assert {:ok, message, :v1_5} = Admi002.decode(xml)
  ...
  test "ref referencia o PI-ResourceId da mensagem recusada" do
    ...
    assert {:ok, xml} = Admi002.encode(message, @header, :v1_5)
    assert {:ok, de_volta, :v1_5} = Admi002.decode(xml)
  ...
  test "campo obrigatório ausente é rejeitado antes de montar XML" do
    ...
    assert {:error, reason} = Admi002.encode(message, @header, :v1_5)
  ...
  test "parse rejeita XML de outra mensagem" do
    ...
    assert {:error, _reason} = Admi002.decode(outro_xml)
  end
end
