defmodule Isox.Camt053Test do
  use ExUnit.Case, async: true

  alias Isox.{AppHdr, Camt053}
  ...
  test "sem saldo nenhum é rejeitado (Bal exige ao menos 1 no schema real)" do
    ...
    assert {:error, reason} = Camt053.encode(message, @header, :v1_4)
    assert reason =~ "Bal"
  end
  ...
  test "com múltiplos tipos de saldo" do
    ...
    assert {:ok, xml} = Camt053.encode(message, @header, :v1_4)
    assert {:ok, de_volta, :v1_4} = Camt053.decode(xml)
  ...
  test "parse rejeita XML de outra mensagem" do
    ...
    assert {:error, _reason} = Camt053.decode(outro_xml)
  end
end
