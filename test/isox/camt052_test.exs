defmodule Isox.Camt052Test do
  use ExUnit.Case, async: true

  alias Isox.{AppHdr, Camt052}
  ...
  test "monta e volta pra struct" do
    assert {:ok, xml} = Camt052.encode(@message, @header, :v1_3)
    assert {:ok, de_volta, :v1_3} = Camt052.decode(xml)
    assert de_volta == @message
  end

  test "campo obrigatório ausente é rejeitado antes de montar XML" do
    message = %{@message | acct_ispb: nil}
    assert {:error, reason} = Camt052.encode(message, @header, :v1_3)
  ...
  test "parse rejeita XML de outra mensagem" do
    ...
    assert {:error, _reason} = Camt052.decode(outro_xml)
  end
end
