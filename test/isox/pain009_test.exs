defmodule Isox.Pain009Test do
  use ExUnit.Case, async: true

  alias Isox.{AppHdr, Pain009}
  ...
  test "monta e volta pra struct, só com o obrigatório" do
    assert {:ok, xml} = Pain009.encode(@message, @header, :v1_1)
    assert {:ok, de_volta, :v1_1} = Pain009.decode(xml)
  ...
  test "campo obrigatório ausente é rejeitado antes de montar XML" do
    message = %{@message | cdtr_name: nil}
    assert {:error, reason} = Pain009.encode(message, @header, :v1_1)
    assert reason =~ "cdtr_name"
  end
  ...
  test "parse rejeita XML de outra mensagem" do
    ...
    assert {:error, _reason} = Pain009.decode(outro_xml)
  end
end
