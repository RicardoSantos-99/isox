defmodule Isox.Pain014Test do
  use ExUnit.Case, async: true

  alias Isox.{AppHdr, Pain014}
  ...
  test "aceite, sem reason, monta e volta pra struct" do
    assert {:ok, xml} = Pain014.encode(@message, @header, :v2_4)
    assert {:ok, de_volta, :v2_4} = Pain014.decode(xml)
  ...
  test "rejeição com reason, e 2.3 também funciona" do
    ...
    assert {:ok, xml} = Pain014.encode(message, @header, :v2_3)
    assert {:ok, de_volta, :v2_3} = Pain014.decode(xml)
