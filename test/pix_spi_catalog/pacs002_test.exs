defmodule PixSpiCatalog.Pacs002Test do
  ...
  test "monta e volta pra struct em 1.16, status de aceite sem reason" do
    assert {:ok, xml} = Pacs002.build(@message, @header, :v1_16)
    assert {:ok, message, :v1_16} = Pacs002.parse(xml)
  ...
  test "rejeição com motivo e informação adicional vai e volta" do
    ...
    assert {:ok, xml} = Pacs002.build(message, @header, :v1_17)
  ...
  test "código de motivo novo em 1.17 (DS02) é rejeitado em 1.16" do
    ...
    assert {:error, _reason} = Pacs002.build(message, @header, :v1_16)
    assert {:ok, _xml} = Pacs002.build(message, @header, :v1_17)
  end

  test "campo obrigatório ausente é rejeitado antes de montar XML" do
    message = %{@message | tx_sts: nil}

    assert {:error, reason} = Pacs002.build(message, @header, :v1_16)
  ...
  test "parse rejeita XML de outra mensagem" do
    ...
    assert {:error, _reason} = Pacs002.parse(outro_xml)
  end
end
