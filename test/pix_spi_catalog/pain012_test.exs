defmodule PixSpiCatalog.Pain012Test do
  ...
  test "aceite, sem motivo nem SplmtryData" do
    assert {:ok, xml} = Pain012.build(@message, @header, :v1_4)
  ...
  test "rejeição com reason" do
    ...
    assert {:ok, xml} = Pain012.build(message, @header, :v1_3)
  ...
  test "campo obrigatório ausente é rejeitado antes de montar XML" do
    ...
    assert {:error, reason} = Pain012.build(message, @header, :v1_4)
  ...
  test "parse rejeita XML de outra mensagem" do
    ...
    assert {:error, _reason} = Pain012.parse(outro_xml)
  end
end
