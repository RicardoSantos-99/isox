defmodule PixSpiCatalog.Pacs008Test do
  ...
  test "monta e volta pra struct em 1.15 (sem InstrId)" do
    assert {:ok, xml} = Pacs008.build(@message, @header, :v1_15)
    assert {:ok, message, :v1_15} = Pacs008.parse(xml)
  ...
  test "monta e volta pra struct em 1.16, com InstrId" do
    ...
    assert {:ok, xml} = Pacs008.build(message, @header, :v1_16)
  ...
  test "InstrId só é aceito na versão 1.16" do
    ...
    assert {:error, reason} = Pacs008.build(message, @header, :v1_15)
    assert reason =~ "1.16"
  end
  ...
  test "campo obrigatório ausente é rejeitado antes de montar XML" do
    ...
    assert {:error, reason} = Pacs008.build(message, @header, :v1_16)
  ...
  test "valor que não bate com o padrão do campo é rejeitado" do
    ...
    assert {:error, _reason} = Pacs008.build(message, @header, :v1_16)
  end
  ...
  test "parse rejeita XML de outra mensagem" do
    ...
    assert {:error, _reason} = Pacs008.parse(outro_xml)
  end
end
